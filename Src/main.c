...
int main(void)
{
  ...
  ili9341_color_t color = ILI9341_WHITE; // Assuming there's a predefined red color
  int16_t x = 20;  // X coordinate of the top-left corner of the rectangle
  int16_t y = 50;  // Y coordinate of the top-left corner of the rectangle
  uint16_t width = 7;  // Width of the rectangle
  uint16_t height = 40;  // Height of the rectangle
  //int16_t y = 0;  // Initial Y coordinate
  ...
  while (1) {
	  ...
	  ili9341_fill_rect(ili9341_display, ILI9341_BLACK, x, y - 1, width, 1);
	  } else if (direction == -1 && y + height < 240) {
	  // If moving up and not at the bottom, clear the area below the new rectangle position
	  ili9341_fill_rect(ili9341_display, ILI9341_BLACK, x, y + height, width, 1);
	  }


	  // Draw the rectangle at the current position
	  ili9341_fill_rect(ili9341_display, color, x, y, width, height);
	  HAL_Delay(5);
